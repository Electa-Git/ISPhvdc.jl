...
""
function run_mp_tnepopf(data::Dict{String,Any}, model_type::Type, solver; ref_extensions = [_PMACDC.add_ref_dcgrid!, _PMACDC.add_candidate_dcgrid!], setting = s, kwargs...)
    ...
    if haskey(data, "nw") && haskey(data["nw"]["1"], "ne_branch") # combined AC and DC TNEP
        ...
        return _PM.solve_model(data, model_type, solver, build_mp_acdctnepopf; multinetwork=true, ref_extensions = [_PMACDC.add_ref_dcgrid!, _PMACDC.add_candidate_dcgrid!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
    elseif haskey(data, "ne_branch")
        if setting["process_data_internally"] == true
            data["dcline"] = Dict{String, Any}()
            _PMACDC.process_additional_data!(data)
        end
        return _PM.solve_model(data, model_type, solver, build_mp_acdctnepopf; multinetwork=false, ref_extensions = [_PMACDC.add_ref_dcgrid!, _PMACDC.add_candidate_dcgrid!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
    else
        print("The data does not have any ne_branch options.")
    end
end
...
function objective_min_tnep_cost(pm::_PM.AbstractPowerModel)
    ...
    return JuMP.@objective(pm.model, Min,
        ...
        sum(
            sum( 
                # gen["construction_cost"] * (_PM.var(pm, n, :pgslackabs, i))
                # +
                gen_cost[(n,i)] 
                for (i,gen) in nw_ref[:gen] 
            )
