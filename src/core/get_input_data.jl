...
function get_demand_data(scenario, year, data_dir)
...
function get_dn_demand_data(scenario, year, data_dir)
...
function get_rez_grid_extensions(data_dir)
...
function get_generator_information(data_dir)
...
# removes extra info from the names of solar and wind generators
...
function get_res_timeseries(year, data_dir)
...
function get_pv_timeseries(year, data_dir)
...
function get_wind_timeseries(year, data_dir)
...
function aggregate_res_timeseries(res_ts, generator_info, type::String)
    ...
    for (plant, profile) in res_ts
        if plant[1:3] !== "REZ" && plant[3] !== '_'
            # find which generator in generator_info nest matches the name of the time series data
            ...
            # aggregate total solar or wind availability for each state
    ...
    # divide the total availability by the number of generators in each state
