function prepare_hourly_opf_data!(hourly_data, grid_data, total_demand, average_demand, pv_series, wind_series, rez_pv, rez_wind, time_stamp)
    ...
    for (g, gen) in hourly_data["gen"]
        trace = 1 # default value (applies to non-REG's)
        ...
        elseif gen["gen_status"] == 1 # WTGs and PV generators outside REZ's
            gen_bus = gen["gen_bus"]
...
function multi_network_uc_data_no_cont(data, total_demand, total_dn_demand, pv_series, wind_series, rez_pv, rez_wind, hours; dn_res_factor=0.5)
    data["pst"] = Dict{String,Any}()
    number_of_hours = length(hours)
    ...
    mn_data["hour_ids"] = hour_ids
    mn_data["number_of_hours"] = number_of_hours
    mn_data["number_of_contingencies"] = 0

    for idx in 1:number_of_hours
        hour = hours[idx]
        for (l, load) in mn_data["nw"]["$idx"]["load"]
            load_bus = load["load_bus"]
            area_code = data["bus"]["$load_bus"]["area"]
            area = data["areas"]["$area_code"]
            wind_dn = total_dn_demand["Wind"][area][hour]
