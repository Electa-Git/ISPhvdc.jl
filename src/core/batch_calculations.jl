function batch_fsuc(mn_data, fmin, dc_solver, scenario, year, h; lean = false, droop = false, extension = "")
    ...
    for idx = 1:length(fmin)
        ...
        if droop == false
            if lean == false
                ...
                t_dc["$idx"] = @elapsed result_dc = CbaOPF.solve_fsuc_lean(mn_data, _PM.DCPPowerModel, dc_solver, setting = s, multinetwork = true)
            end
        ...
        if droop == false
            if lean == false
                ...
                t_no_dc["$idx"] = @elapsed result_no_dc = CbaOPF.solve_fsuc_lean(mn_data, _PM.DCPPowerModel, dc_solver, setting = s, multinetwork = true)
            end
...
function batch_fsrdopf(data_dict, fmin, dc_solver, scenario, year, hours, h, uc_result; droop = true, extension = extension)
    ...
    for idx = 1:length(fmin)
        ...
        for h_idx in 1:length(hours)
            ...
            print("=====================================================================================================", "\n")
            hour = hours[h_idx]
            mn_data = multi_network_uc_data(data, total_demand_series, dn_demand_series, pv_series, wind_series, pv_rez, wind_rez, hour, generator_contingencies, no_dc_cont = no_dc_cont, dn_res_factor = dn_res_factor, p2p = p2p, uc_result = uc_result, uc_hour = h_idx)
            print("==================================== DC ====================================================", "\n")
            if droop == false
                s = Dict("output" => Dict("branch_flows" => true), "conv_losses_mp" => true, "hvdc_inertia_contribution" => true, "relax_uc_binaries" => true, "use_gen_status" => true)
                time_dc = @elapsed r_dc = CbaOPF.solve_fsrdopf(mn_data, _PM.DCPPowerModel, dc_solver, setting = s, multinetwork = true)
            else
                s = Dict("output" => Dict("branch_flows" => true), "conv_losses_mp" => true, "hvdc_inertia_contribution" => true, "relax_uc_binaries" => true, "use_gen_status" => true, "use_droop" => true )
                time_dc = @elapsed r_dc = CbaOPF.solve_fsrdopf(mn_data, _PM.DCPPowerModel, dc_solver, setting = s, multinetwork = true)
            end
            result_dc["objective"] = result_dc["objective"] + r_dc["objective"]
            result_dc["solution"]["nw"]["$h_idx"]  = r_dc["solution"]
            ...
            print("==================================== NO DC =================================================", "\n")
            if droop == false
                s = Dict("output" => Dict("branch_flows" => true), "conv_losses_mp" => true, "hvdc_inertia_contribution" => false, "relax_uc_binaries" => true, "use_gen_status" => true)
                time_no_dc = @elapsed r_no_dc = CbaOPF.solve_fsrdopf(mn_data, _PM.DCPPowerModel, dc_solver, setting = s, multinetwork = true)  
            else
                s = Dict("output" => Dict("branch_flows" => true), "conv_losses_mp" => true, "hvdc_inertia_contribution" => false, "relax_uc_binaries" => true, "use_gen_status" => true, "use_droop" => true)
                time_no_dc = @elapsed r_no_dc = CbaOPF.solve_fsrdopf(mn_data, _PM.DCPPowerModel, dc_solver, setting = s, multinetwork = true)
            end
            result_no_dc["objective"] = result_no_dc["objective"] + r_no_dc["objective"]
            result_no_dc["solution"]["nw"]["$h_idx"] = r_no_dc["solution"]
