...
# "variable: `pdslack[i]` for `i` in `load`s"
# function variable_load_power_real_slack(pm::_PM.AbstractPowerModel; nw::Int=_PM.nw_id_default, bounded::Bool=false, report::Bool=true)
#     pdslack = _PM.var(pm, nw)[:pdslack] = JuMP.@variable(pm.model,
#         [i in _PM.ids(pm, nw, :load)], base_name="$(nw)_pdslack",
#         start = _PM.comp_start_value(_PM.ref(pm, nw, :load, i), "pdslack_start")
#     )
#     report && _PM.sol_component_value(pm, nw, :load, :pdslack, _PM.ids(pm, nw, :load), pdslack)
# end


# function variable_load_power_real_slack_abs(pm::_PM.AbstractPowerModel; nw::Int=_PM.nw_id_default, bounded::Bool=false, report::Bool=true)
#     pdslackabs = _PM.var(pm, nw)[:pdslackabs] = JuMP.@variable(pm.model,
#         [i in _PM.ids(pm, nw, :load)], base_name="$(nw)_pdslackabs",
#         start = _PM.comp_start_value(_PM.ref(pm, nw, :load, i), "pdslackabs_start", 0.0)
#     )
#     report && _PM.sol_component_value(pm, nw, :load, :pdslackabs, _PM.ids(pm, nw, :load), pdslackabs)
# end


# "variable: `qdslack[i]` for `i` in `load`s"
# function variable_load_power_imaginary_slack(pm::_PM.AbstractPowerModel; nw::Int=_PM.nw_id_default, bounded::Bool=false, report::Bool=true)
#     qdslack = _PM.var(pm, nw)[:qdslack] = JuMP.@variable(pm.model,
#         [i in _PM.ids(pm, nw, :load)], base_name="$(nw)_qdslack",
#         start = _PM.comp_start_value(_PM.ref(pm, nw, :load, i), "qdslack_start")
#     )
#     report && _PM.sol_component_value(pm, nw, :load, :qdslack, _PM.ids(pm, nw, :load), qdslack)
# end

# function variable_load_power_imaginary_slack_abs(pm::_PM.AbstractPowerModel; nw::Int=_PM.nw_id_default, bounded::Bool=false, report::Bool=true)
#     qdslackabs = _PM.var(pm, nw)[:qdslackabs] = JuMP.@variable(pm.model,
#         [i in _PM.ids(pm, nw, :load)], base_name="$(nw)_qdslackabs",
#         start = _PM.comp_start_value(_PM.ref(pm, nw, :load, i), "qdslackabs_start", 0.0)
#     )
#     report && _PM.sol_component_value(pm, nw, :load, :qdslackabs, _PM.ids(pm, nw, :load), qdslackabs)
# end

# ""
# function constraint_load_slack_abs(pm::_PM.AbstractPowerModel, i::Int; nw::Int=_PM.nw_id_default)
#     constraint_load_slack_abs(pm, nw, i)
# end

# ""
# function constraint_load_slack_abs(pm::_PM.AbstractPowerModel, n::Int, i::Int)
#     pdslack = _PM.var(pm, n, :pdslack, i)
#     pdslackabs = _PM.var(pm, n, :pdslackabs, i)
#     # qdslack = _PM.var(pm, n, :qdslack, i)
#     # qdslackabs = _PM.var(pm, n, :qdslackabs, i)
#     JuMP.@constraint(pm.model, pdslack <= pdslackabs)
#     JuMP.@constraint(pm.model, -pdslack <= pdslackabs)
#     # JuMP.@constraint(pm.model, qdslack <= qdslackabs)
#     # JuMP.@constraint(pm.model, -qdslack <= qdslackabs)
# end
